...
function Card({ title, label, description, assignee, doneAt }: { title: string; label: string; description: string; assignee: string; doneAt: Date }) {

    const labelColor = label === 'design' ? 'blue' : label === 'development' ? 'green' : 'yellow';
  return (
    <div className="flex flex-col gap-1 p-3 bg-white rounded shadow ">
